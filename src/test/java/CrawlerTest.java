...
public class CrawlerTest {
    ...
    //Beginning of method Crawler.initiateSingleItemSearch tests.
    ...
    public void ifAnInexistantTypeOfMediaIsPutInAsArgumentOfFindSingleItemMethodThrowAnExceptionTest() {
        ...
        try {
            crawler.initiateSingleItemSearch(invalidItemType, properItemName, baseUrl);
        ...
        Assert.fail("After calling the initiateSingleItemSearch() method of the Crawler class with an invalid item type argument, " +
    ...
    public void ifTheInputTypePassedToTheFindSingleItemMethodIsSupportedDontThrowAnExceptionTest(String validItemType) {
        try {
            crawler.initiateSingleItemSearch(validItemType, properItemName, baseUrl);
        } catch (IllegalArgumentException e) {
    ...
    public void ifAnEmptyStringIsGivenAsItemNameInputWhenLookingForASingleItemThrowExceptionTest() {
        try {
            crawler.initiateSingleItemSearch(properItemType, "", baseUrl);
        } catch (IllegalArgumentException e) {
        ...
        Assert.fail("After calling the initiateSingleItemSearch() method of the Crawler class an empty itemName argument, an IllegalArgumentException should have been thrown.");
    }

    @Test(expected = IllegalArgumentException.class)
    public void ifAnEmptyStringIsGivenAsURLArgumentForFindSingleItemMethodThrowExceptionTest() {
        try {
            crawler.initiateSingleItemSearch(properItemType, properItemName, "");
        } catch (IllegalArgumentException e) {
        ...
        Assert.fail("After calling the initiateSingleItemSearch() method of the Crawler with an empty string baseUrl argument, an IllegalArgumentException should have been thrown.");
    }

    @Test
    public void noExceptionsThrownWhenCorrectInputIsGivenToTheFindSingleItemMethod() {
        try {
            crawler.initiateSingleItemSearch(properItemType, properItemName, baseUrl);
        } catch (IllegalArgumentException e) {
            Assert.fail(String.format("The arguments passed to the Crawler.initiateSingleItemSearch (%1$s, %2$s, %3$s) were correct, but an IllegalArgumentException was thrown with message %3$s", properItemType, properItemName, baseUrl, e.getMessage()));
        } catch (Exception e) {
            Assert.fail(String.format("Unexpected %1$s thrown. Exception message: %2$s", e.getClass(), e.getMessage()));
        }
    }
    ...
    //End of method Crawler.initiateSingleItemSearch tests.
    ...
    public void ifALegitURLIsPassedAsArgumentToFindAllItemsDontThrowExceptionTest() {
        try {
            ...
            Assert.fail("The method Crawler.findAllItems was passed a valid parameter. It should not have thrown an exception. Parameter: " + baseUrl + " Exception: " + e.getMessage());
        }
    }
